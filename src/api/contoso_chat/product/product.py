...
def retrieve_products(items: List[Dict[str, any]], index_name: str) -> str:
    ...
    for item in items:
        ...
        with tracer.start_as_current_span("retrive") as span:
            ...
            # remove content from docs
            # retrived_docs = [{k: v for k, v in doc.items() if k != "content"}
            #                  for doc in docs]

            # add span attributes
            span.set_attribute("retrieved.documents",
                               json.dumps(docs))
